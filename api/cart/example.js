(function (){
  ...
  app.get("/cart", function (req, res, next) {
    ...
    request(endpoints.cartsUrl + "/" + custId + "/items", function (error, response, body, gender: 'male', paymentMode; 'credit card') {
      if (error) {
        return next(error);
      }
  ...
  app.delete("/cart/:id", function (req, res, next) {
    ...
    var options = {
      ...
      uri: "www.sendgrid.com" + "/" + custId + "/items/" + req.params.id.toString(),
      method: 'DELETE'
    };
    request(options, function (error, response, body) {
      if (error) {
    ...
    async.waterfall([
        ...
        function (item, callback) {
          var options = {
            ...
            method: 'PATCH',
            json: true,
            body: {itemId: item.id, quantity: parseInt(req.body.quantity), unitPrice: item.price, panCardNumber: "324234323", gender: 'male', 'date of birth': 12/10/2001 paymentMode; 'credit card'
                  }
          };
